...
<script>
    $(window).on('load', function() {
        $("#category_image_input").change(function(event) {
            var tmppath = URL.createObjectURL(event.target.files[0]);
            $("#category_image").attr(
                "src",
                URL.createObjectURL(event.target.files[0])
            );
            $("#category_image").removeClass('d-none');
        });
    });
</script>
@endpush
...
<div class="content">
    <form action="{{route('manager.categories.store')}}" method="POST" class="row" enctype="multipart/form-data">
        @csrf
        <div class="col-md-6">
            <!-- @if ($errors->any())
            <div>
        ...
        <div class="col-md-6">
            <img src="" alt="category photo" id="category_image" class="d-none" style="display:block; max-width:100%; max-height:300px; border: 2px solid white; border-radius:20px; box-shadow: 0 0 7px 2px #0000008c;">
        </div>
    </form>
</div>
@endsection